...
def run_one_iteration(
    ...
    if iteration == 1:
        ...
        posterior_mean_maximizers = torch.tensor(
            np.atleast_2d(
                np.loadtxt(
                    results_folder + "posterior_mean_maximizers/posterior_mean_maximizers_" + str(trial) + ".txt"
                )
            )
        )
        runtimes = list(
    ...
    if iteration == 1:
        ...
        posterior_mean_maximizers = torch.cat((posterior_mean_maximizers, posterior_mean_maximizer))
